import psycopg2
from configparser import ConfigParser
from sqlalchemy import create_engine

import pandas as pd


class DBConnector:

    def __init__(self, **kwargs):
        """
        :param filename: (str) default: 'database.ini'
        :param section: (str) default: 'postgresql'
        """
        self.params = self.config(**kwargs)
        self.engine = None
        self.cur = None
        self.conn = None

    def connect(self):
        """ Connect to the PostgreSQL database server """
        try:
            # connect to the PostgreSQL server
            print('[DB_CLIENT] Connecting to the PostgreSQL database...')
            self.conn = psycopg2.connect(**self.params)

            # create a cursor
            self.cur = self.conn.cursor()
            print('[DB_CLIENT] Connected to the PostgreSQL database...')

        except (Exception, psycopg2.DatabaseError) as error:
            print(error)

    def disconnect(self):
        """ Disconnect from the PostgreSQL database server """
        try:
            print('[DB_CLIENT] Disconnecting from the PostgreSQL database...')
            self.cur.close()

        except (Exception, psycopg2.DatabaseError) as error:
            print(error)

        finally:
            if self.conn is not None:
                self.conn.close()
                print('[DB_CLIENT] Connection Closed.')

    def execute_command(self, commands):
        """ Execute query """
        try:
            self.connect()
            # execute command one by one
            for command in commands:
                self.cur.execute(command)

            # commit the changes
            self.conn.commit()
        except (Exception, psycopg2.DatabaseError) as error:
            print(error)
        finally:
            self.disconnect()

    def get_df(self, command):
        """ Execute query and return result """
        try:
            self.connect()
            # execute command one by one
            df = pd.read_sql()

            # commit the changes
            self.conn.commit()
        except (Exception, psycopg2.DatabaseError) as error:
            print(error)
        finally:
            self.disconnect()

    @staticmethod
    def config(filename='database.ini', section='postgresql'):
        # create a parser
        parser = ConfigParser()
        # read config file
        parser.read(filename)

        # get section, default to postgresql
        db = {}
        if parser.has_section(section):
            params = parser.items(section)
            for param in params:
                db[param[0]] = param[1]
        else:
            raise Exception('Section {0} not found in the {1} file'.format(section, filename))

        return db
